...
DysLexer.prototype.next = function() {
  ...
  // Ok the above character matching could have switches the scope
  // it's actually the only job the character matching has...
  // but that character matching has a purpose.
  // however because we have switches scopes at _this_ exact point
...
DysLexer.prototype.fireToken = function() {
  ...
  if(this.token) {
    ...
    if(!this.level[this.rootScope].onToken(this.token)) {
