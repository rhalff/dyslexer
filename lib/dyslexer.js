...
var DysLexer = function () {
  function DysLexer(rootScope) {
    _classCallCheck(this, DysLexer);
  ...
  _createClass(DysLexer, [{
    ...
    key: 'toScope',
    value: function toScope(scope, c) {
    ...
    value: function next() {
      ...
      } else if (this.tokenEnding.indexOf(c) === -1 && this.eol.indexOf(c) === -1) {
        /** default behaviour, build the token */
        if (!this.tokenStart) this.tokenStart = this.current;
        this.token += c;
      } else if (
      /** don't act if the scope took control */
      this.level[this.scope].tokenEnding === undefined && this.tokenEnding.indexOf(c) >= 0) {
        this.fireToken();
      }
      ...
      this.level[this.scope].tokenEnding === undefined && this.eol.indexOf(c) >= 0) {
        // fire last token
        this.fireToken();
    ...
    value: function fireToken() {
      if (this.token) {
        ...
        if (this.level[this.scope].tokensExpected < this.level[this.scope].tokens.length) {
          throw new Error(['Unexpected token count for', this.scope, 'expected', this.level[this.scope].tokensExpected, 'instead of', this.level[this.scope].tokens.length, 'tokens\n\n\t', JSON.stringify(this.level[this.scope].tokens, null, 2), '\n'].join(' '));
        }
    ...
    key: 'present',
    value: function present(name, data) {
      /** validate the token */
      if (this.level[this.scope].validate) {
        ...
        if (reg && !reg.test(data)) {
          throw new Error(['Invalid', name, 'token does not match', reg.toString(), '\n\n\t', data, '\n'].join(' '));
        }
      }
    ...
    key: 'checkStructure',
    value: function checkStructure(scope) {
      ...
      if (tokens.length) {
        ...
        if (s.structure.length) {
          ...
          for (var str in s.structure) {
            if ((0, _compare2.default)(tokens, s.structure[str])) {
              match = true;
              break;
            }
          }
          if (!match) {
            throw new Error(['Structure mismatch for', scope, JSON.stringify(tokens), 'did not match any of', JSON.stringify(s.structure), '\n\n\t', JSON.stringify(s.tokens)].join(' '));
          }
...
DysLexer.Scope = _scope2.default;
