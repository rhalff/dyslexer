...
function Matcher(chars) {
  ...
  this.level.RootScope.onLineStart();
...
Matcher.prototype.next = function() {
  ...
  // There is a difference between line start and end.
  // ofcourse there is no differents if you do not reset
  // any data, but line end should still contain any data
  // from that line, line start has this data reset.
  // So first determine what to reset.
  // it makes especially sense when starting the root scope.
  // the first one, because it has to act upon it.
